...
class TFIMModelGenerator(BaseComponent):
    ...
    def _validate_physical_parameters(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if num_qubits < 2:
            ...
            validated["num_qubits"] = 20  # 限制最大系统大小（避免经典模拟困难）
        else:
...
if __name__ == "__main__":
    print("🧪 Testing TFIMModelGenerator...")
    ...
    try:
        ...
        # 测试用例 - 模拟ParameterMatcher已经处理过的标准化参数
        test_cases = [
            {
            ...
            },
            {
                ...
                "params": {"num_qubits": 6, "J": 2.0}  # h和topology需要默认值
            },
            {
                "name": "String parameters (from LLM)",
                "query": "TFIM VQE optimization", 
                "params": {"num_qubits": "8", "J": "1.2", "h": "0.6"}  # LLM可能返回字符串
            },
            {
            ...
            },
            {
                ...
                "params": {"num_qubits": 25, "J": -2.0, "h": 15.0}  # 测试边界条件
            },
            {
        ...
        print(f"\n✅ All TFIMModelGenerator tests passed!")
        print(f"🎯 Component demonstrates correct LLM-driven architecture:")
        print(f"  • Trusts upstream ParameterMatcher for semantic parsing")
        print(f"  • TFIM-specific physical parameter validation")
        print(f"  • Safe type conversion and boundary checking")
        print(f"  • Intelligent TFIM default value assignment")
        print(f"  • Comprehensive TFIM parameter documentation")
        print(f"  • Clear separation of concerns (no parameter name mapping)")
        
    except Exception as e:
        print(f"⚠️ TFIMModelGenerator test error: {e}")
        import traceback
        traceback.print_exc()