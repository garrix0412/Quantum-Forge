...
Consider:
1. VQE/TFIM workflow (Model → Hamiltonian → Circuit → Estimator → Optimization → Execution)
2. QAOA/MaxCut workflow (Graph → Hamiltonian → Circuit → Optimization) - NO separate Estimator needed
3. QML workflow (Dataset → Circuit → Network → Training → Evaluation)
4. HHL/Linear System workflow (Preprocessor → State Preparation → QPE → Controlled Rotations → Inverse QPE → Measurement)
5. Grover workflow (Problem → Circuit → Optimization → Verification)
6. Problem-specific requirements (TFIM, QAOA, MaxCut, Grover, QML binary classification, HHL linear system, etc.)
7. Component dependencies and logical flow
...
JSON:"""
    ...
    def _fallback_component_selection(self, intent: QuantumIntent) -> List[str]:
        ...
        elif "QML" in problem_type or "MACHINE LEARNING" in problem_type or "CLASSIFICATION" in problem_type:
            return [name for name in self._component_cache.keys() 
                   if "QML" in name]
        elif "HHL" in problem_type or "LINEAR SYSTEM" in problem_type:
            return [name for name in self._component_cache.keys() 
                   if "HHL" in name]
        else:
            # 返回通用组件
